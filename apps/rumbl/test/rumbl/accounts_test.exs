defmodule Rumbl.AccountsTest do
  ...
  describe "registration_user/1" do
    @valid_attrs %{
      name: "User",
      username: "eva",
      password: "secret"
    ...
    test "with valid data inserts user" do
      assert {:ok, %User{id: id}=user} = Accounts.registration_user(@valid_attrs)
    ...
    test "with invalid data does not insert user" do
      assert {:error, _changeset} = Accounts.registration_user(@invalid_attrs)
      assert Accounts.list_users() == []
    end

    test "enforces unique usernames" do
      assert {:ok, %User{id: id}} = Accounts.registration_user(@valid_attrs)
      assert {:error, changeset} = Accounts.registration_user(@valid_attrs)
    ...
    test "does not accept long usernames" do
      ...
      {:error, changeset} = Accounts.registration_user(attrs)
